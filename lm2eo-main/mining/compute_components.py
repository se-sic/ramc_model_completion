...
def get_graph_components(graphs, filtered=False, filter_config=None):
    ...
    for graph in graphs:
        ...
        filtered = {'too_large': 0, 'too_many_similar': 0}
    if filtered:
...
def filter_too_large(components: list, filtered: dict, nb_nodes=50, nb_edges=80):
    new_components = []
...
def get_components(input_folder, formatting=INPUT_FORMAT_NX, filtered=False):
    ...
    if formatting == INPUT_FORMAT_NX:
        ...
        components, nb_of_components_per_diff = load_components_networkx(input_folder)
        if filtered:
            components, filtered_total = filter_too_large(components, filtered=dict())
        else:
            filtered_total = {'too_large': 0, 'too_many_similar': 0}
    ...
    return components, nb_of_components_per_diff, filtered_total['too_large'] 

def has_node(graph, label):

    return label in [node[1]['label'] for node in list(graph.nodes(data=True))]
    
def main(input_folder, output_folder, max_components_per_file=200, formatting=INPUT_FORMAT_NX):
    ...
    return components, nb_of_components_per_diff, filtered
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 4:
        ...
        main(sys.argv[1], sys.argv[2], int(sys.argv[3]), formatting = sys.argv[4])	
    else:
