...
def load_components_networkx(folder):
    components = []
...
def import_tlv_folder(folder_path, postfix='.lg', is_directed=True, parse_support=True, auto_migrate=True):
    '''
    ...
    for file in os.listdir(folder_path):
        if file.endswith(postfix):
            new_graphs = import_tlv(os.path.join(folder_path, file), is_directed, parse_support, auto_migrate=auto_migrate)
            if parse_support:
...
'''
Tooling to migrate the old label format to the new, JSON-like format. 
'''
